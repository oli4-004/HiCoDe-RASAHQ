...
class ValidateRouteForm(FormValidationAction):
    ...
    def normalize(self, raw: Text, restrict_to: List[str] | None = None):
        ...
        Returns a dict from LLMController.normalize_building().
        """
        llm = LLMController()
    ...
    def clear_disambiguation_states(self):
        ...
        """
        return [
            SlotSet("followup_question", None),
    ...
    async def validate_origin_building(
        ...
        """
        Normalize + fill slot origin_building
        """

        target = tracker.get_slot("disambiguation_target_slot")
        restrict_to = tracker.get_slot("disambiguation_candidates") or []
        in_disambiguation_for_origin = (
            target == "origin_building" and len(restrict_to) > 0
        )
        ...
        # convert SlotSet(...) objects to {slot_name: value}
        return {
            event.key: event.value for event in events if isinstance(event, SlotSet)
        }
    ...
    async def validate_destination_building(
        ...
        """
        Normalize + fill slot destination_building
        """

        target = tracker.get_slot("disambiguation_target_slot")
        restrict_to = tracker.get_slot("disambiguation_candidates") or []
        in_disambiguation_for_destination = (
            target == "destination_building" and len(restrict_to) > 0
        )
...
class ActionRouteSummary(Action):
    ...
    def run(
        ...
        dispatcher.utter_message(
            text=f"(debug) computing route from '{origin}' to '{dest}' …"
        )
        return []
